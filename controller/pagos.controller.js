...
exports.findAll = (req, res) => {
	sequelize.query("SELECT `monto`, `ref`, `date` ,`users`.`unit` FROM `pagos` INNER JOIN  (SELECT `grupoVecinalIdgrupo`, `uid`,`unit` FROM `usuario_grupos` INNER JOIN `usuarios` ON `usuario_grupos`.`usuarioUid` = `usuarios`.`uid` AND `usuario_grupos`.`grupoVecinalIdgrupo` = :idgrupo ) AS `users` ON `users`.`uid` = `pagos`.`usuarioUid` AND `users`.`grupoVecinalIdgrupo` = `pagos`.`grupoVecinalIdgrupo` AND YEAR(`pagos`.`date`) = :year AND MONTH(`pagos`.`date`) = :mes",
	{ replacements: { idgrupo: req.params.idgrupo, year: req.params.year, mes: req.params.mes },  type: sequelize.QueryTypes.SELECT} ).then(user=>{
		res.json(user);
	});
};
